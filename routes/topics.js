exports.get_topics = function(req, res){
    if(models.User && req.session.user_sess) {
        step(
        function() {
            models.User.find({ linkedin_id : req.session.user_sess.id }, this);
        },
        function(err, result) {
            var topics = {};
            if(_.size(result)) {
...
exports.save_topics = function(req, res) {
  step(
    ...
    function(err, result) {
        ...
        models.User.update({ linkedin_id : result[0].linkedin_id },
